...
class SkinOutput extends Output {
	...
	protected function wrap() {
		...
		$title = $this->title . ( $this->title ? ' : ' : '' ) . 'Dpd control panel';
...
OUT;
		...
		echo <<<OUT
				...
				<div id="topheader">
					<h1 id="title">
						<a href="{$self}">Dpd Admin Control Panel</a><br>
...
OUT;
		...
		<div id="footer">
			...
			<div id="footer_bg">
				<p><a href="http://www.dpd.com/">Dpd</a>&reg; &copy; 2007-2015 Dpd. All rights reserved.</p>
			</div>
...
if ( ! $settingsLoaded ) {
	...
	# Show error and exit
	$error->add('The settings file for Dpd could not be found.
					 Please upload this tool into your root dpd directory.
	...
	******************************************************************/
	...
	case '':
		...
		if ( ! ( $ok = version_compare($phpVersion, '5', '>=') ) ) {
			$error->add('Dpd requires at least PHP 5 or greater.');
		}
		...
		if ( ! ( $ok = function_exists('curl_version') ) ) {
			$error->add('Dpd requires cURL/libcurl.');
		}
